...
import { saveClothToWardrobe, ClothAnalysisResult } from "@/features/wardrobe/api/saveClothToWardrobe";
...
export default function ClothScanScreen() {
  ...
  const [analysisResult, setAnalysisResult] = useState<ClothAnalysisResult | null>(null);
    ...
    if (!cameraPermission?.granted) {
      return (
        <View className="flex-1 bg-[#0A0A0B] items-center justify-center">
          ...
          <TouchableOpacity onPress={requestCameraPermission} className="bg-[#1D9E75] px-6 py-3 rounded-full">
    ...
    return (
      <View className="flex-1 bg-black">
        ...
        <View className="absolute bottom-10 w-full flex-row justify-around items-center px-6">
          <TouchableOpacity onPress={() => setIsCameraOpen(false)} className="bg-gray-800 p-4 rounded-full">
            <Text className="text-white">Cancel</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={handleCameraCapture} className="bg-[#1D9E75] w-20 h-20 rounded-full border-4 border-white" />
        </View>
        {isLoading && (
      ...
      {isLoading ? (
        <View className="flex-1 items-center justify-center">
          ...
          <View className="mt-8 bg-gray-900 p-4 rounded-xl max-w-[80%]">
             <Text className="text-gray-400 text-center">💡 Tip: Ensure good lighting for better AI results.</Text>
          </View>
        </View>
      ) : analysisResult ? (
        <ScrollView className="flex-1 px-4 pt-10" contentContainerStyle={{ paddingBottom: 100 }}>
          ...
          <Text className="text-white text-xl font-bold mb-4">Item Details</Text>
          ...
          {Object.keys(formState).map((key) => (
            <View key={key} className="mb-4">
              ...
              <TextInput
                ...
                value={formState[key] || ""}
                onChangeText={(val) => setFormState({ ...formState, [key]: val })}
          ...
          >
            <Text className="text-white font-bold text-lg">✅ Save to Wardrobe</Text>
          </TouchableOpacity>
        </ScrollView>
      ) : (
        <View className="flex-1 items-center justify-center px-6">
          <Text className="text-white text-2xl font-bold mb-8">Add to Wardrobe</Text>
          
          <TouchableOpacity 
            onPress={() => setIsCameraOpen(true)}
            className="bg-[#1D9E75] w-full py-4 rounded-xl flex-row justify-center items-center mb-4"
          >
            <Text className="text-white font-bold text-lg">📷 Scan from Camera</Text>
          </TouchableOpacity>
          ...
          >
            <Text className="text-white font-bold text-lg">🖼️ Pick from Gallery</Text>
          </TouchableOpacity>
        </View>
      )}
    </View>
