...
| `full-length-pics` | ✅     | Onboarding full-length photos        | created live (used by `onboarding/full-length-pics.tsx`) |

- **Hardened `20260810180000`:** both buckets now enforce `file_size_limit = 10 MB` + `allowed_mime_types = image/*`. `try-on-uploads` INSERT policy is now `authenticated`-only with `auth.jwt() IS NOT NULL` (was PUBLIC/anon → anyone could upload). Public SELECT kept for read-back of try-on results.
...
| ---------------------- | -------------------------------------------- | -------------------------------------- | --------------------------------------------------------------------- |
| `gemini-proxy`         | Proxies Gemini `generateContent`             | —                                      | **RL 30/min/user**                                                    |
| `planner-agent`        | Gemini outfit planner chat (JSON mode)       | — (reads context)                      | **RL 30/min/user**; does **not** persist recommendations              |
| `analyze-cloth-item`   | Cloudinary upload + Gemini clothing analysis | `wardrobe_items` (via app), Cloudinary | **RL 5/min/user** (multi-step: ≤4 Gemini + remove.bg)                 |
| `cloth-label-scan`     | Care-label OCR via Gemini                    | `saved_labels` (via app), Cloudinary   | **RL 10/min/user**; uses `_shared/careSymbols.ts`                     |
| `cloudinary-signature` | Server-side signed upload params             | —                                      |                                                                       |
| `remove-bg`            | remove.bg proxy                              | —                                      | **RL 10/min/user** (each = 1 remove.bg credit)                        |
| `virtual-try-on`       | fal.ai try-on                                | `virtual_try_on_generations`           | **RL 5/min/user** (paid fal.ai); `verify_jwt = true` in `config.toml` |

> **Rate limits (`20260810` hardening):** the 6 functions above call `_shared/rate-limit.ts` — a fixed-window Upstash Redis counter keyed `rl::<fn>::<jwt-sub>`, **fail-open** by design (unconfigured/erroring Upstash ⇒ requests pass). Limits are per-user/min with `Retry-After: 60`. `verify-purchase` + `billing-webhook` **deleted** — they only wrote to the dropped billing tables (see gap #3).

### RPC functions

| Function                                        | Defined in                                | Used by app?         | Notes                                                                                                                                                                                                                                                    |
| ----------------------------------------------- | ----------------------------------------- | -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `get_dashboard_stats(time_filter text)`         | `20260808030000` + `20260810180000` (fix) | ❌ No                | SECURITY DEFINER; fixed `20260810180000` — `auth.uid()` → `auth.jwt() ->> 'sub'` (was 22P02 for Clerk), pinned `search_path=''`, anon revoked (authenticated-only). `v_is_premium` still hardcoded `false` → `itemLimit` always `100` (product decision) |
| `check_username_available(check_username text)` | ✅ `20260810180000` (was missing)         | ✅ `nickname.tsx:34` | SECURITY DEFINER recreated with pinned `search_path=''` + qualified refs; `EXECUTE` revoked from PUBLIC/anon → **authenticated-only** (was a public username-enumeration oracle)                                                                         |
...
🔒 **Hardened `20260810183000`:** the reaction/like trigger functions (`handle_new_reaction`, `handle_updated_reaction`, `handle_deleted_reaction`, `handle_post_like` — all SECURITY DEFINER) were anon-executable via `/rest/v1/rpc` with an unpinned `search_path`. Recreated with `SET search_path = ''` + fully-qualified refs; `EXECUTE` revoked from `PUBLIC`/`anon` (kept for `authenticated`/`service_role` so the triggers still fire on app writes). Same for the `rls_auto_enable` event trigger; `update_updated_at_column`/`update_entitlement_updated_at` pinned for hygiene.
...
| Wardrobe       | `features/wardrobe/model/user-wardrobe-store.ts` — `syncWithDatabase()` (bounded `.range(0,499)`), realtime via `shared/realtime/manager.ts` (singleton)                                                | `wardrobe_items`                                                                                                                                                         |
...
| Social         | `features/social/api/useCommunityPosts.ts` — CRUD + likes/reactions/comments/saves. **No realtime channel** (was removed `20260810`) — pull-to-refresh + optimistic; cached via `use-supabase-query.ts` | `community_posts`, `post_likes`, `post_reactions`, `post_saves`, `post_comments` (+ Cloudinary)                                                                          |
| Notifications  | `features/social/api/useNotifications.ts` — realtime via `shared/realtime/manager.ts` (singleton)                                                                                                       | `notifications` (join `user_profiles!actor_id`, `community_posts!post_id`)                                                                                               |
...
| `supabase/migrations/2026081017*`/`1018*`  | ✅ **Authoritative** | 2026-08-10 hardening: `20260810170000` perf indexes + realtime publication; `20260810180000` RPC + storage security; `20260810183000` trigger lockdown. Applied live via `execute_sql` (MCP `apply_migration` classifier was down) — files tracked here for local `supabase db push` |
...
`20260715000000` wardrobe-scan cols → `20260717000000` saved_labels → `20260719000000` notification cols (`fcm_token`, `notifications_enabled`) → `20260721000000` planned_events → `20260721000001` notifications + reaction triggers → `20260723000000` try-on bucket → `20260730000000` redesign (logged_outfits, outfit_items, gamification, badges, ai_recommendations) → `20260804094631` billing service-role RLS → `20260804205500` indexes → `20260808000000` complete schema → `20260808010000` drop billing tables + re-FK + recreate social policies → `20260808020000` link notifications/post_likes FKs → `20260808030000` dashboard-stats RPC → `20260808040000` drop bio/about → `20260810093000` multi-reaction unique constraint → `20260810110000` fix reaction-trigger join + recreate `notifications`/`post_likes` RLS + create `analytics_logs` → `20260810152136` social-RLS recreation (recorded live) → `20260810160812` fix all owner-RLS policies for Clerk auth (drop `auth.uid()` branch — 55 policies / 19 tables) → `20260810170000` hardening: hot-path indexes (notifications/post_comments/post_saves/post_likes/analytics_logs) + drop duplicate `community_posts` indexes + realtime publication adds `notifications`/`wardrobe_items` → `20260810180000` security: `check_username_available` + `get_dashboard_stats` recreated (pinned `search_path`, Clerk-safe, anon revoked), storage buckets 10MB/image-only, `try-on-uploads` INSERT authenticated-only → `20260810183000` trigger lockdown (reaction/like SECURITY DEFINER fns: pinned `search_path`, anon revoked).

**Realtime design (`20260810170000`):** `supabase_realtime` publication now contains `user_profiles` + `notifications` + `wardrobe_items` — the **personal** channels the client subscribes to. `community_posts` is **intentionally NOT published** (public feed → fan-out amplification; the feed uses pull-to-refresh + optimistic updates instead — see the `useCommunityPosts` rework). Do not add it without a replay/pagination plan. The pre-fix state (only `user_profiles` published) meant every `postgres_changes` subscription on the other tables paid polling load but delivered nothing.
...
| 3   | **Billing edge functions write to dropped tables**                     | ~~`verify-purchase` / `billing-webhook` targeted `entitlements`/`purchase_tokens`/`billing_events`~~                                                                                                                      | ✅ **Fixed `20260810`** — both edge functions **deleted** from repo. **Deployed copies still need removal in the Supabase dashboard** (Settings → Functions) — I couldn't reach the delete API.                                             |
| 4   | **`get_dashboard_stats` premium flag hardcoded `false`**               | `v_is_premium := false` → `itemLimit` always `100`; RPC not called by the app                                                                                                                                             | ✅ Security fixed `20260810180000` (Clerk-safe `auth.jwt() ->> 'sub'`, pinned `search_path`, authenticated-only). Premium wiring remains a product decision — wire to RevenueCat entitlement or drop the RPC.                               |
| 5   | **`check_username_available` RPC missing from migrations**             | Used at `src/app/(root)/onboarding/nickname.tsx:34`                                                                                                                                                                       | ✅ **Fixed `20260810180000`** — now defined in a migration (SECURITY DEFINER, pinned `search_path`, `EXECUTE` **authenticated-only**; was a public username-enumeration oracle). Fresh environments now work.                               |
