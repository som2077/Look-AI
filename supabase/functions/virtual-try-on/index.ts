...
serve(async (req) => {
  ...
  try {
    const { person_image_url, garment_image_url, garment_photo_type, garment_category } = await req.json();

    if (!person_image_url || !garment_image_url) {
      return new Response(
        JSON.stringify({ error: 'person_image_url and garment_image_url are required' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    // Map footwear to auto for fal.ai compatibility
    const mappedCategory = garment_category === 'footwear' ? 'auto' : garment_category;
    ...
    if (falData.images && falData.images.length > 0) {
      ...
      return new Response(
        JSON.stringify({ error: 'Unexpected response structure from fal.ai', data: falData }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
